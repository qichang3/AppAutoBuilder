...
def extractLastError(lastError):
    ...
    elif lastError.__contains__("Javadoc generation failed"):
        errorMsg.javaDoc_generate_failed = True
        isFound = 1
        errorInfo1 = errorMsg.javaDoc_generate_Msg(isFound)

    elif lastError.__contains__("dx tool not found at"):
        errorMsg.dx_found_question = True
        isFound = dxFileExtraction(lastError)
