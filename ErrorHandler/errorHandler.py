...
def errorHandler(errorInfo):
    ...
    if errorInfo['errorType'] == 'repository_connect_failed': #仓库连接错误
        ...
        if errorInfo['errorFlg'] == 1:
            jvmHeapHandler()
    elif errorInfo['errorType'] == 'javaDoc_generate_failed':
        if errorInfo['errorFlg'] == 1:
            javaDocGenerateHandler(errorInfo)
    elif errorInfo['errorType'] == 'dx_found_question':
        if errorInfo['errorFlg'] == 1:
            copyExistingDx(errorInfo)
    else:
        fixingSuccess = False
    print("fixing end")
    return fixingSuccess

def copyExistingDx(errorInfo):
    dxFile = errorInfo['dx_file_path']
    os.chdir(config.android_sdk_home)
    dx_file_list = os.popen("find -name 'dx'").readlines()
    if len(dx_file_list) > 0:
        dx_file_source = dx_file_list[0][:-1]
        os.system('cp ' +  dx_file_source + ' ' + dxFile)
    os.chdir(config.download_path)

def javaDocGenerateHandler(errorInfo):
