    ...
    for line in sublinelist[1:]:
        if line.__contains__("No value has been specified for property "):
            ...
            if property != None:
                ...
                property_list.append(property.group()[1:-1])
    if len(property_list) > 1:  # error_log103
...
def evaluateRootProject(sublinelist):
    isFound = 0
    project_name = re.search("'([^\"]*)'", sublinelist[0], flags=0)
    if project_name != None:
        config.project_name = project_name.group()[1:-1]
        isFound = 1
    if sublinelist[1].__contains__("Could not find property"):  # error_log16 and error_log97
        ...
        if len(property_name) == 1:
            ...
            if sublinelist[1].__contains__("on root project"):
                ...
                isFound = 4
    elif sublinelist[1].__contains__("No such property"):
    ...
    for line in sublinelist[1:]:
        if (line.__contains__("There were failing tests.") or
            ...
            break
              ...
              line.__contains__("Could not resolve all task dependencies for configuration")):
